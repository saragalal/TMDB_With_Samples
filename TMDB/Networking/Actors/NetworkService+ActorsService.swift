...
extension NetworkManager {
    ...
    func getSearchedActors(pageNumber:Int,searchText: String ,completion: @escaping (
        _ result: Swift.Result<APIResponse<Person>, NetworkError>,
        _ statusCode: StatusCode?
        ) -> Void) {
        
        provider.request(MultiTarget(ActorsService.search(page: pageNumber, text: searchText))) { (result) in
            switch result {
            case .success(let response):
                if (200...299 ~= response.statusCode) {
                    do {
